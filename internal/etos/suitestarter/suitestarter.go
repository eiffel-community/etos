...
func (r *ETOSSuiteStarterDeployment) Reconcile(ctx context.Context, cluster *etosv1alpha1.Cluster) error {
	...
	cfg, err := r.reconcileConfig(ctx, logger, secret.Name, suiteStarterName, cluster)
	if err != nil {
	...
	_, err = r.reconcileDeployment(ctx, logger, cfg.Name, suiteRunnerTemplateName, suiteStarterName, cluster)
	if err != nil {
