...
var _ = Describe("ExecutionSpace Webhook", func() {
	var (
		obj       *etosv1alpha2.ExecutionSpace
		oldObj    *etosv1alpha2.ExecutionSpace
		defaulter ExecutionSpaceCustomDefaulter
	...
	BeforeEach(func() {
		...
		obj = &etosv1alpha2.ExecutionSpace{}
		oldObj = &etosv1alpha2.ExecutionSpace{}
		defaulter = ExecutionSpaceCustomDefaulter{FakeReader(&environmentRequest, nil)}
	...
	Context("When creating ExecutionSpace under Defaulting Webhook", func() {
		It("Should apply defaults when a required field is empty", func() {
			...
			Expect(obj.Labels).To(Equal(map[string]string{
				...
				"app.kubernetes.io/name":                                 "executionspace-provider",
			}))
		})
	})
