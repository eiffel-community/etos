...
ETOS strives to be as deterministic as possible in regards to how it executes tests. In the test suite file it shall be defined exactly what to run and how to run it and ETOS' job is to execute it exactly according to that specification.
For even greater determinism it is recommended to always have absolute values in the test suite, such as using the commit ID (sha-1) instead of branches when checking out tests.
...
This is where the separation of concerns in ETOS comes to play. With ETOS we are trying to hide the complexity that you don't need to worry about and let you worry about what you know.
